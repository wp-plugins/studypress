...
?> 
    ...
    &nbsp;&nbsp;&nbsp;&nbsp;<a href="admin.php?page=id_sub2"><input style="" type="submit" value="Add new" name="add_newcat" title="Add new category" class="button-primary" /></a></br></br>
      <?php  if ( isset($_POST['add_new_cat']) ) { 
		...
		$nbre = 0;
		          		        global $wpdb;
            foreach( $wpdb->get_results($wpdb->prepare("select cat_parent from $studi_category where cat_id='".$_POST['cat_par']."'")) as $key => $row) {
...
if ( isset($_POST['update_cat']) ){
	    ...
	    if($_POST['cat_par'] ==Neither){
          ...
          $req11=$wpdb->prepare("UPDATE $studi_category SET cat_name = '".$cat_name."',cat_des='".$cat_desc."',cat_parent=NULL WHERE cat_id='".$_GET['id_cat']."'");
	}
				 else {
          $req11=$wpdb->prepare("UPDATE $studi_category SET cat_name = '".$cat_name."',cat_des='".$cat_desc."',cat_parent='".$cat_par."' WHERE cat_id='".$_GET['id_cat']."'");
  ...
  &nbsp;&nbsp;&nbsp;&nbsp;<input style="" type="submit" value="Add a new category" name="add_new_cat" title="Add a new category" class="button-primary" />
    <?php }
     ...
     global $wpdb;
                         foreach( $wpdb->get_results($wpdb->prepare("SELECT cat_id,cat_name,cat_des,cat_parent FROM $studi_category where cat_id='".$_GET['id_cat']."'")) as $key => $row) {
                // each column in your row will be accessible like this
  ...
  global $wpdb;
  $req5=$wpdb->prepare("UPDATE $studi_category SET cat_parent =NULL WHERE cat_parent IN (".$categ_list.")");
  $res5=$wpdb->query($req5);
  $req=$wpdb->prepare("DELETE FROM $studi_categ_cours WHERE cat_id IN(".$categ_list.")");
  $wpdb->query($req); 
  $req=$wpdb->prepare("DELETE FROM $studi_categ_quiz WHERE cat_id IN(".$categ_list.")");
  $wpdb->query($req);   
  $req2=$wpdb->prepare("DELETE FROM $studi_category WHERE cat_id IN(".$categ_list.")");
  $wpdb->query($req2);}
  ?>
...
<?php
			 foreach( $wpdb->get_results($wpdb->prepare("SELECT count(*) as nbr FROM $studi_category")) as $key => $row)
             {$cat_nbr = $row-> nbr;} 
 if($cat_nbr==0){   ?>
...
<?php
  ...
  foreach( $wpdb->get_results($wpdb->prepare("SELECT count(*) as nbr
                               ...
                               WHERE cat_id = '".$cat_id."'")) as $key2 => $row2)
  ...
  foreach($res2 as $key1 => $row1 ){
    ...
    foreach( $wpdb->get_results($wpdb->prepare("SELECT count(*)as nbre
                               FROM $studi_categ_cours
                               WHERE cat_id='".$cat_id1."'")) as $key => $row)
                               {$catf_cours_nbr = $row-> nbre;}
  foreach( $wpdb->get_results($wpdb->prepare("SELECT count(*) as nbr
                               FROM $studi_categ_quiz
                               WHERE cat_id = '".$cat_id1."'")) as $key2 => $row2)
                               {$cat_quiz_nbr = $row2-> nbr;}
          echo '<table cellpadding="5" cellspacing="0"  border="2" class="widefat">
            <tr>
